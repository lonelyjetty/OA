...
(function(undefined) {
	...
	ace.vars['touch'] = ('ontouchstart' in window);// (('ontouchstart' in
													// document.documentElement)
													// || (window.DocumentTouch
													// && document instanceof
													// DocumentTouch));

	// sometimes the only good way to work around browser's pecularities is to
	// detect them using user-agents
...
(function($, undefined) {
	// sometimes we try to use 'tap' event instead of 'click' if jquery mobile
	// plugin is available
...
jQuery(function($) {
		...
		if ($.fn.ace_sidebar_scroll)
			$sidebar.ace_sidebar_scroll({
				...
				'include_toggle' : false || ace.vars['safari']
						|| ace.vars['ios_safari'] // true = include toggle
													// button in the scrollbars
			});
	...
	function enableDemoAjax() {
		...
		var demo_ajax_options = {
			...
			'content_url' : function(hash) {
				// ***NOTE***
				// this is for Ace demo only, you should change it to return a
				// valid URL
				...
				// /ajax/index.html#page/gallery to > /ajax/content/gallery.html
				// and load it
				if (path.match(/(\/ajax\/)(index\.html)?/))
					return path.replace(/(\/ajax\/)(index\.html)?/,
							'/ajax/content/' + hash.replace(/^page\//, '')
									+ '.html');

				// for example in Ace PHP demo version we convert
				// "ajax.php#page/dashboard" to "ajax.php?page=dashboard" and
				// load it
				return path + "?" + hash.replace(/\//, "=");
			}
		}

		// for IE9 and below we exclude PACE loader (using conditional IE
		// comments)
		// for other browsers we use the following extra ajax loader options
		if (window.Pace) {
			demo_ajax_options['loading_overlay'] = 'body';// the opaque
															// overlay is
															// applied to 'body'
		}
		...
		// if general error happens and ajax is working, let's stop loading icon
		// & PACE
	...
	function handleScrollbars() {
		...
		if (has_scroll && !ace.vars['old_ie']) {// IE has an issue with widget
												// fullscreen on ajax?!!!
			$(window).on(
					'resize.reset_scroll',
					function() {
	...
	function sidebarTooltips() {
		...
		/**
		 * $('.sidebar').tooltip({ 'placement': function (context, source) { var
		 * offset = $(source).offset();
		 * 
		 * if( parseInt(offset.left) < parseInt(document.body.scrollWidth / 2) )
		 * return 'right'; return 'left'; }, selector: '.nav-list
		 * .badge[title],.nav-list .label[title]', container: 'body', template: '<div
		 * class="tooltip tooltip-error"><div class="tooltip-arrow"></div><div
	...
	function someBrowserFix() {
		// chrome and webkit have a problem here when resizing from 479px to
		// more
		...
		// fix an issue with ios safari, when an element is fixed and an input
		// receives focus
							...
							.each(
									function() {
										...
										if (($icon_down = $icon
											...
											$icon_up = $icon
													.attr('data-icon-hide')
										} else if ($match = $icon.attr('class')
												.match(/fa\-(.*)\-(up|down)/)) {
											$icon_down = 'fa-' + $match[1]
													+ '-down'
											$icon_up = 'fa-' + $match[1]
													+ '-up'
										}
	...
	function smallDeviceDropdowns() {
		...
		function adjustNavbarDropdown() {
			...
			if ($sub.css('position') == 'fixed') {
				...
				// we set 'width' here for text wrappings and spacings to take
				// effect before calculating scrollHeight
				$sub.css('width', width);
				...
				if (dropdown_content.length == 1) {
					// sometimes there's no scroll-content, for example in
					// detached scrollbars
				...
				$sub.css('height', height + (!tabbed ? 2 : 7));// for bottom
																// border
																// adjustment
																// and tab
																// content
				...
				if ($sub.hasClass('user-menu')) {
					$sub.css('height', '');// because of user-info
											// hiding/showing at different
											// widths, which changes above
											// 'scrollHeight', so we remove it!

					// user menu is re-positioned in small widths
					// but we need to re-position again in small heights as well
					// (modal mode)
	...
	$$.getAttrSettings = function(el, attr_list, prefix) {
		...
		for ( var li in attr_list)
			if (attr_list.hasOwnProperty(li)) {
				...
				var attr_val, attr_name = $$.unCamelCase(
						name.replace(/[^A-Za-z0-9]{1,}/g, '-')).toLowerCase()
