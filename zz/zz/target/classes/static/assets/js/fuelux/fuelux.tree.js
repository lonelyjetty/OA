 ...
 * 
 * Copyright (c) 2014 ExactTarget Licensed under the BSD New license.
 */
...
(function(factory){
  ...
  var Tree = function(element, options){
    ...
    // this.$element.on('click.fu.tree', '.tree-branch-name', $.proxy(
    // function(ev) { this.openFolder(ev.currentTarget); }, this));
    ...
    // ACE
    /**
     * if( this.options.folderSelect ){ this.$element.off('click.fu.tree',
     * '.tree-branch-header'); this.$element.on('click.fu.tree', '.icon-caret',
     * $.proxy( function(ev) { this.openFolder($(ev.currentTarget).parent()); },
     * this)); this.$element.on('click.fu.tree', '.tree-branch-header', $.proxy(
     * function(ev) { this.selectFolder($(ev.currentTarget)); }, this)); }
     */
  ...
  Tree.prototype = {
      ...
      this.options.dataSource(treeData ? treeData : {}, function(items){
        ...
        $.each(items.data, function(index, value){
          ...
          if (value.type === 'folder') {
            $entity = self.$element.find('[data-template=treebranch]:eq(0)').clone().removeClass('hide').removeAttr(
                'data-template');
            $entity.data(value);
            ...
            if ('additionalParameters' in value && 'item-selected' in value.additionalParameters
            ...
            $entity = self.$element.find('[data-template=treeitem]:eq(0)').clone().removeClass('hide').removeAttr(
                'data-template');
    ...
    selectItem : function(el){
      ...
      if (this.options.multiSelect) {
        ...
        //
        $all.removeClass('tree-selected').find('i').removeClass(this.options['selected-icon']).addClass(
            this.options['unselected-icon']);// ACE
        data.push(selData);
      }
      
      var eventType = 'selected';
      if ($el.hasClass('tree-selected')) {
        //sdevil507 2016-6-8 modify
        //ace-tree 新增cancelSelect属性,用于设置选中的item再次点击是否可以取消
        //默认是不取消,但是在multiSelect:true情况下,需要开启,因为多选情况下需要有取消操作
        if (true == this.options.cancelSelect) {
    ...
    openFolder : function(el){
      ...
      // ACE
      /**
       * if (!this.options.folderSelect) { $el = $(el).parent(); // tree-branch,
       * if tree-branch-name clicked }
       */
    ...
    collapse : function(){
      ...
      // find open folders
      this.$element.find('.' + $.trim(this.options['open-icon'].replace(/(\s+)/g, '.'))).each(
          function(){// ACE
            // update icon class
            var $this = $(this).removeClass(this.options['open-icon'] + ' ' + this.options['close-icon']).addClass(
